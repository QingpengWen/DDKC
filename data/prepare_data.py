...
def prepare_assistments(data_name, min_interactions_per_user, remove_nan_skills, train_split=0.8):
    ...
    df = pd.read_csv(os.path.join(data_path, "preprocessed_data.csv"), encoding="ISO-8859-1")
...
def prepare_kddcup10(data_name, min_interactions_per_user, kc_col_name, remove_nan_skills, train_split=0.8):
    ...
    df = pd.read_csv(os.path.join(data_path, "preprocessed_data.csv"), delimiter='\t')
...
def prepare_nips(data_name, min_interactions_per_user=10, remove_nan_skills=True, train_split=0.8):
    # data_path = "data/nips34"
    data_path = os.path.join("", data_name)
    data = pd.read_csv(os.path.join(data_path, "prelevel_data_train.csv"), encoding="ISO-8859-1")
    # np.loadcsv(os.path.join(data_path, ""), dtype=int)
    
    df = data.rename(columns={"answer_timestamp": "timestamp",
                            "UserId": "user_id",
                            "QuestionId": "item_id",
                            "SubjectId_level3_str": "skill_id",
                            "IsCorrect": "correct"})
    df["timestamp"] = df["timestamp"] - df["timestamp"].min()
    ...
    # np.savetxt(os.path.join(data_path, "bkt_expert_labels.txt"), bkt_skills, fmt='%i')
    # np.savetxt(os.path.join(data_path, "bkt_splits.txt"), bkt_split, fmt='%i')
    # df = pd.DataFrame(data=data, columns=("UserId", "QuestionId", "correct"))
    # 
    # skills = np.loadtxt(os.path.join(data_path, "spanish_expert_labels.txt"))
    # df["skill_id"] = skills[df["item_id"]].astype(np.int64)
    # 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='assistments09')  # algebra05, assistments09, assistments12, assistments15, assistments17, statics 
    parser.add_argument('--min_interactions', type=int, default=10)
    # parser.add_argument('--remove_nan_skills', action='store_true')
    parser.add_argument('--remove_nan_skills', action='store_true', default=True)
